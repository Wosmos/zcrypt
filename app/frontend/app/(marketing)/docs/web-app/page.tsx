  ...
  return (
    ...
    >
      ...
      <DocSection id="vault-lock" title="The vault lock">
        ...
        <DocList
          items={[
            ...
            <>
              By default a session unlock lasts <strong>15 minutes</strong> of activity and then
              auto-locks, dropping the passphrase and any decrypted data from memory.
            </>,
          ]}
        />
        <DocNote type="info">
          Folders you protect with their own password are the one exception: they ask for that
          folder&apos;s password the first time you open them in a session. See{" "}
          <Link
        ...
        <DocList
          items={[
            <>
              ...
              <strong>IndexedDB</strong>, never in <code>localStorage</code> and never as plaintext.
            </>,
            <>
              It&apos;s encrypted with an AES-GCM key that is <strong>non-extractable</strong>: the
              browser can use the key to decrypt but its raw bytes can never be read back out, even
              by zcrypt&apos;s own code.
            </>,
            <>
      ...
      <DocSection id="notifications" title="Background notifications">
        ...
        <DocNote type="info">
          Notifications are entirely local to your browser and purely informational. Denying the
          permission has no effect on transfers; you just won&apos;t get the desktop pop-up.
        </DocNote>
      </DocSection>
