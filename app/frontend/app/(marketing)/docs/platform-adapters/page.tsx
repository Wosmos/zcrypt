  ...
  return (
    ...
    >
      <DocSection id="byo" title="Bring your own storage">
        ...
        <DocP>
          Because it&apos;s your account, your usable space is bounded only by that platform&apos;s
          free space rather than by a plan we sell. zcrypt handles the encryption, chunking, and
          repository management on top.
        </DocP>
      </DocSection>
      ...
      <DocSection id="tokens" title="Tokens & scopes">
        <DocP>
          ...
          <strong>Settings → Platform Tokens</strong>. Grant the least privilege that still lets
          zcrypt create repositories and read/write files:
        </DocP>
        <DocTable
        ...
        <DocNote type="security" title="Tokens are encrypted at rest">
          Every platform token is encrypted with AES-256-GCM using a server-side key-encryption key
          (KEK) before it touches the database. Even in a database breach, your tokens stay
          protected.
        </DocNote>
      </DocSection>
      ...
      <DocSection id="telegram" title="Telegram's token format">
        <DocP>
          Telegram has no concept of repositories, so its &quot;token&quot; is actually two values
          joined with a pipe: your bot token and the target chat or channel ID.
        </DocP>
...
123456:ABC-DEF1234ghIkl-zyx57W2v1u123ew11|@my_zcrypt_vault`}</DocCode>
        <DocList
          items={[
            ...
            <>
              Make a <strong>private channel or group</strong> to hold your files and add the bot to
              it as an administrator.
            </>,
            <>
              Use the channel&apos;s <code>@username</code> or its numeric ID as{" "}
              <code>CHAT_ID</code>, then paste <code>BOT_TOKEN|CHAT_ID</code> into zcrypt.
            </>,
          ]}
        />
      </DocSection>
      ...
      <DocSection id="reachability" title="When a backend can't be reached">
        ...
        <DocList
          items={[
            <>
              In <strong>Settings → Platform Tokens</strong> the account still shows as connected
              but is flagged <strong>unreachable</strong>, with a short reason such as
              &quot;unreachable: connection timed out&quot;, &quot;unreachable: DNS lookup
      ...
      <DocSection id="multiple" title="Multiple accounts & managed storage">
        ...
        <DocP>
          ...
          <strong>global / managed</strong>, so users can store files without connecting any account
          of their own. When no personal token and no managed storage is available yet, uploads
          return a clear &quot;storage not available&quot; message instead of failing silently.
        </DocP>
      </DocSection>
