  ...
  return (
    ...
    >
      <DocNote type="warning" title="Sign-in with GitHub ≠ GitHub as storage">
        <p className="mb-2">These are two unrelated things that both mention GitHub:</p>
        <p className="mb-2">
          <strong>Signing in with GitHub</strong> (this page) uses GitHub as an identity provider to
          log you into your zcrypt account. It grants zcrypt nothing beyond your basic profile and
          email.
        </p>
        <p>
          <strong>Connecting GitHub as storage</strong> is a separate step where you add a personal
          access token in settings so zcrypt can store your encrypted file chunks in a repository.
          That is covered in{" "}
          <Link
      ...
      <DocSection id="sign-in" title="Signing in with a provider">
        <DocP>
          Choose <strong>Continue with Google</strong> or <strong>Continue with GitHub</strong> on
          the login page. zcrypt redirects you to the provider, you approve, and the provider sends
          you back. From there one of three things happens:
        </DocP>
        <DocList
          items={[
            <>
        ...
        <DocNote type="security" title="Why the verified-email rule matters">
          If a zcrypt account exists for an email that was never verified, zcrypt refuses to
          auto-link a provider to it and asks you to log in with your existing method first, then
          link in settings. This stops someone who controls a Google/GitHub account for an
          unverified address from quietly taking over the matching local account. Tokens are also
          returned to the browser in the URL <em>fragment</em>, so they never land in server logs or
          referrer headers.
        </DocNote>
      </DocSection>
      ...
      <DocSection id="linking" title="Linking & unlinking">
        ...
        <DocNote type="info">
          ...
          account, your files, or any storage backend you connected.
        </DocNote>
      </DocSection>

      <DocSection id="desktop" title="Desktop sign-in">
        <DocP>
          The desktop app signs you in through your normal <strong>system browser</strong> rather
          than an embedded webview, so you authenticate in a trusted, full-featured browser you
          already use. The flow:
        </DocP>
        <DocList
          ordered
          items={[
