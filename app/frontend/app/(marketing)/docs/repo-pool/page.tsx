    ...
    >
      <DocSection id="why" title="Why a pool">
        ...
        <DocP>
          ...
          Connect two GitHub accounts and each gets its own independent pool; they never share or
          contend.
        </DocP>
      </DocSection>
      ...
      <DocSection id="rotation" title="How rotation works">
        <DocP>
          At any moment, each pool has exactly one <strong>active</strong> repo that new chunks go
          to. The cycle is simple:
        </DocP>
        <DocList
          ordered
          items={[
            ...
            <>
              Before each upload it checks the active repo&apos;s tracked usage against the platform
              threshold.
            </>,
            <>
              When the active repo reaches its threshold, zcrypt <strong>deactivates</strong> it and{" "}
              <strong>auto-creates a new repository</strong> to become the active one.
            </>,
            <>New chunks flow into the new repo; the cycle repeats.</>,
          ]}
        />
        <DocP>
          New repositories are created private and given a plausible, developer-looking name such as{" "}
          <code>fast-cache-v3</code> or <code>core-worker-v7</code>, with a description like
          &quot;Internal build artifacts and cache storage.&quot; Nothing in the repo hints that it
          belongs to an encrypted drive. On Telegram, which has no repos, the same logic spreads
          files across virtual storage locations in your channel instead.
        </DocP>
        <DocNote type="info" title="Old repos stay readable">
          Deactivating a full repo only stops new writes to it. Its chunks are still tracked and
          downloaded normally, so files you stored months ago keep working as the pool grows around
          them.
        </DocNote>
      </DocSection>
      ...
      <DocSection id="thresholds" title="Per-platform thresholds">
        <DocP>
          Thresholds are deliberately conservative, sitting under each platform&apos;s real limits
          so uploads never bump into a hard wall mid-transfer.
        </DocP>
        <DocTable
        ...
        <DocNote type="warning" title="Hugging Face doesn't grow by rotating">
          Hugging Face&apos;s free tier is 100 GB for the entire account, not per repo, so spinning
          up another Hugging Face repo adds no real space. The per-repo threshold is kept under that
          account-wide ceiling, and when zcrypt picks a backend automatically it prefers Telegram
          (no ceiling) and lists Hugging Face last. To grow Hugging Face capacity, connect another
          account.
        </DocNote>
      </DocSection>
      ...
      <DocSection id="throttling" title="Push throttling">
        <DocP>
          Some platforms limit how fast you can push, not just how much you can store. GitHub, for
          instance, throttles sustained pushes at roughly <strong>7 GB per hour</strong>. To avoid
          tripping that ceiling mid-transfer, zcrypt paces its own writes to each platform rather
          than hammering the API and getting rate-limited.
        </DocP>
        <DocP>
        ...
        <DocList
          items={[
            <>
              Only GitHub is throttled by default (~7 GB/hour); Telegram, GitLab, and Hugging Face
              are not rate-limited by zcrypt.
            </>,
            <>
              Budgets are tracked <strong>per platform</strong>, so saturating GitHub never slows a
              push to GitLab or Telegram.
            </>,
            <>
              The wait happens <em>before</em> a chunk claims a repo upload slot, so a throttled
              push never holds a slot idle or blocks other repos from making progress.
            </>,
          ]}
        />
      </DocSection>
      ...
      <DocSection id="tracking" title="Usage tracking">
        <DocP>
          zcrypt records the encrypted bytes written to each repo as uploads complete, so the
          &quot;is this repo full?&quot; check is a fast database lookup rather than a live API call
          on every chunk. Usage is tracked <strong>per repo</strong>, which is what lets a pool know
          precisely when to rotate and lets your total capacity add up across all of them.
        </DocP>
      </DocSection>
      ...
      <DocSection id="durability" title="Resume & chunk cleanup">
        <DocSubsection title="Server-side resume">
          <DocP>
            Chunks are staged on the server and pushed to your backend by a background worker, so a
            transfer doesn&apos;t depend on your browser staying open for every byte. If the server
            restarts, the worker re-drains any chunks still waiting and finishes pushing them.
          </DocP>
          <DocP>
        ...
        <DocSubsection title="Cleaning chunks off the platform">
          ...
          <DocP>
            Failed deletions are retried with backoff. If a chunk still can&apos;t be removed after
            repeated attempts it&apos;s flagged in the logs as an orphan needing manual cleanup,
            rather than being silently forgotten, and a startup sweep clears staging files whose
