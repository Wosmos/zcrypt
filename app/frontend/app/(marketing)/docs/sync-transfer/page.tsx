  ...
  return (
    ...
    >
      <DocSection id="offline" title="Offline pins">
        <DocP>
          Offline pins let you mark specific files to keep available on a given device. Pinning is{" "}
          <strong>per device</strong>: each browser is given its own random device id (kept locally
          on that device), and your pins are recorded against that device &mdash; so every device
          keeps its own list, and pinning or unpinning on one doesn&rsquo;t change the others.
        </DocP>
        <DocList
          items={[
            <>
              <strong>Pick a device:</strong> the tab shows the current device&rsquo;s short id, so
              you can tell which device you&rsquo;re pinning for.
            </>,
            <>
              <strong>Pin or unpin:</strong> toggle any file in your vault to add or remove it from
              that device&rsquo;s offline list.
            </>,
            <>
              <strong>Stays private:</strong> a pin only records <em>which</em> file you want kept
              on hand &mdash; the file itself stays encrypted, exactly as it is in your vault.
            </>,
          ]}
        />
      </DocSection>
      ...
      <DocSection id="clipboard" title="Encrypted clipboard sync">
        <DocP>
          Clipboard sync keeps a short, shared history of small snippets available across the
          sessions where you&rsquo;re signed in. Add a snippet on one device and it appears on the
          others in real time, ready to copy.
        </DocP>
        <DocList
          items={[
            <>
              <strong>Item types:</strong> text and links. A snippet that is a lone{" "}
              <code>http(s)</code> URL is tagged as a link; everything else is text. The web tool
              captures typed or pasted text only &mdash; there is no image capture.
            </>,
            <>
            ...
            <>
              ...
              <strong>24 hours</strong> after you add it, and only your most recent snippets are
              kept (the server retains the latest 30).
            </>,
          ]}
        />
      </DocSection>
      ...
      <DocSection id="clipboard-how" title="How clipboard sync works">
        <DocP>
          Each snippet is encrypted <strong>in your browser</strong> with a 256-bit AES-GCM key
          before it&rsquo;s sent. That key is generated on your device and stored only in the
          browser &mdash; it never leaves the device and is never sent to the server. When you add a
          snippet, the server stores only the <strong>ciphertext</strong> plus lightweight metadata
          (its id, type, size, and timestamp) and pushes a live <strong>SSE</strong> (server-sent
          events) &ldquo;clipboard&rdquo; event to your other signed-in sessions so they can pull
          it. Fetching a snippet returns the ciphertext, which is decrypted locally.
        </DocP>
        <DocNote type="security" title="What the server sees">
          The server only ever holds the snippet as <strong>ciphertext</strong> and the metadata it
          needs to wake up your other sessions. It never receives the encryption key and cannot read
          a snippet&rsquo;s contents.
        </DocNote>
        <DocNote type="info" title="Each device needs the same key">
          Because the key lives only in the browser and isn&rsquo;t synced by the server, a device
          has to hold the matching key to read a snippet. A device without it will show a
          &ldquo;different device key&rdquo; notice instead of the plaintext &mdash; the trade-off
          of keeping the key entirely on your side.
        </DocNote>
      </DocSection>
      ...
      <DocSection id="folders" title="Folder sync">
        <DocP>
          ...
          <strong>zcrypt desktop client</strong> should back up automatically. The web app is only
          for <strong>configuration</strong>: you add a folder path and an optional label, and
          toggle each one active or paused. The browser never reads or uploads the files in those
        ...
        <DocList
          items={[
            ...
            <>
              ...
              two devices, both must be connected at the same time. If the connection drops
              mid-transfer, just start a new room.
            </>,
          ]}
        />
        <DocNote type="info" title="Recipient not online?">
          Device-to-device transfer needs both ends connected together. If you need someone to pick
          a file up later, use{" "}
          ...
          instead &mdash; both store the encrypted file so the recipient can fetch it on their own
          schedule.
        </DocNote>
      </DocSection>
