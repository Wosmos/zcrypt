  ...
  return (
    ...
    >
      ...
      <DocSection id="docker" title="Run with Docker">
        <DocP>
          Build the image from the repository root, then run it with your configuration passed as
          environment variables. At minimum you need a database connection, a master key, a JWT
          secret, and your frontend origin.
        </DocP>
...
docker run -p 8080:8080 \\
  ...
  zcrypt`}</DocCode>
        <DocP>
          Prefer an env file? Pass <code>--env-file app/backend/.env</code> instead of repeating{" "}
          <code>-e</code> flags. The container listens on <code>8080</code> by default; hosts like
          Railway inject a <code>PORT</code> variable, which the server also honours.
        </DocP>
      ...
      <DocSection id="required" title="Required environment variables">
        <DocP>
          These must be set for a production deployment. The server refuses to start without{" "}
          <code>DATABASE_URL</code> and <code>MASTER_KEY</code>.
        </DocP>
        <DocTable
          ...
          rows={[
            ...
            [
              ...
              <>
                32-byte hex key (64 hex chars) for envelope encryption of platform tokens at rest.
            ...
            [
              ...
              <>
                Secret used to sign access and refresh tokens (HS256). Must be at least 32
                characters. If left empty the server auto-generates an ephemeral one, so set a
                stable value in production or tokens stop validating after a restart.
              </>,
            ],
            [
            ...
            [
              ...
              <>
                Comma-separated list of origins allowed by CORS. Defaults to <code>localhost</code>{" "}
                if unset; <code>FRONTEND_URL</code> is always added on top.
              </>,
            ],
          ]}
        />
      ...
      <DocSection id="optional" title="Optional environment variables">
        <DocP>
          Everything below is optional. OAuth and email each switch on only when their full set of
          values is present.
        </DocP>
        <DocTable
          head={["Variable", "Description"]}
          rows={[
            ...
            [
              ...
              <>
                Port to listen on (default <code>8080</code>). <code>PORT</code> is also honoured
                for cloud hosts that inject it.
              </>,
            ],
            [
              <code key="v">ZCRYPT_TRUSTED_PROXY_COUNT</code>,
              <>
                Number of trusted reverse-proxy hops in front of the app. The real client IP is read
                that many hops in from the right of the <code>X-Forwarded-For</code> chain.{" "}
                <code>0</code> (default) ignores forwarding headers so clients cannot spoof their IP
                to bypass rate limits; set <code>1</code> behind a single proxy such as Railway.
              </>,
            ],
            [
            ...
            [
              ...
              <>
                From address for emails, e.g. <code>zcrypt &lt;noreply@example.com&gt;</code>.
              </>,
            ],
            [
        ...
        <DocNote type="info" title="Email is optional, but recommended">
          Without <code>RESEND_API_KEY</code> the server runs fine, but it cannot send verification
          or password-reset emails. If you configure email, both the API key and the from-address
      ...
      <DocSection id="oauth" title="OAuth (Google / GitHub)">
        <DocP>
          Sign-in with Google or GitHub is optional. A provider is enabled only when both its client
        ...
        />
        <DocP>
          Register these exact redirect URIs with each provider, substituting your real{" "}
...
https://<BACKEND_URL>/api/auth/oauth/github/callback`}</DocCode>
        <DocP>
          The backend logs the exact URIs to register at startup, and serves them (with no secrets)
          at <code>GET /api/auth/oauth/config</code> so you can verify the live configuration.
        </DocP>
      </DocSection>

      <DocSection id="frontend" title="Frontend">
        <DocP>
          The frontend only needs to know where the backend lives. Point it at your running backend,
          then run the dev server or produce a production build.
        </DocP>
