...
func (e *UploadEngine) Upload(ctx context.Context, filePath, passphrase string, progressFn func(UploadProgress)) error {
	...
	//    encryption). Chunks are encrypted with the random CEK; the CEK is
	//    wrapped with the passphrase-derived KEK and sent on init.
	...
	wrappedCekB64 := base64.StdEncoding.EncodeToString(wrappedCek)
